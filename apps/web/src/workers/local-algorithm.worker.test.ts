...
describe("local algorithm worker protocol", () => {
  it("refines a native grayscale ROI without a network request", () => {
    ...
    const response = handleLocalCommand({ type: "refine", frame: { frame: 0, timestampMs: 0, width: 25, height: 25, source: "image" }, patch: { width: 25, height: 25, data }, roi: { x: 100, y: 200, width: 25, height: 25 }, intent: "circle-center" });
    expect(response.type).toBe("refinement");
    if (response.type === "refinement") expect(response.result.roi.x).toBe(100);
  });
});
