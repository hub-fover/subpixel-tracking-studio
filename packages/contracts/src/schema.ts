...
export const MultiPointTrackSchema = z.object({
  ...
  predictionSource: z.enum(["reference-homography", "keyframe-homography", "local-affine", "previous-position", "manual-recovery"]).default("previous-position"),
  innovationPx: z.number().nonnegative().default(0),
...
export const FrameRegistrationSchema = z.object({
  ...
  method: z.enum(["sift-ransac", "orb-ransac", "homography", "none", "sift-homography", "orb-homography", "adjacent-flow", "translation-fallback"]),
  inlierCount: z.number().int().nonnegative(),
  matchCount: z.number().int().nonnegative(),
  inlierRatio: z.number().min(0).max(1),
  medianReprojectionError: z.number().nonnegative(),
  reprojectionErrorSemantics: z.enum(["pixel-reprojection", "not-available"]).optional(),
  ...
  inverseTransform: z.object({ kind: z.enum(["homography", "affine"]), matrix: z.array(z.number().finite()).length(9) }).optional(),
  fundamentalMatrix: z.array(z.number().finite()).length(9).optional(),
...
export const RiskNoticeSchema = z.object({
  ...
  code: z.enum([
    ...
    "tracking.local-affine-degenerate",
    "tracking.marker-ambiguous",
